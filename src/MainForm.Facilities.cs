...
public partial class MainForm {
    ComboBox manualFacilityCombo;CheckBox manualShowAll;Label manualInfoLabel;
    ...
    void EditFacilitySlots(string selected){
        if(!CanEdit())return;using(var d=new EditDialog("시설별 슬롯 설정")){d.Height=580;var controls=new Dictionary<string,NumericUpDown>();foreach(var name in Facilities.Names){var n=Number(Math.Min(50,Facilities.Total(cfg,name)),50);controls[name]=n;d.Add(name,n);}d.Hint("각 시설의 실제 슬롯 수를 입력하세요. 0칸으로 설정하면 해당 시설은 자동 가공에서 제외됩니다.");if(d.ShowDialog(this)!=DialogResult.OK)return;foreach(var pair in controls)cfg.FacilitySlots[pair.Key]=(int)pair.Value.Value;Save();RenderAll();}
    }
    void EditGoal(Goal old){
        if(!CanEdit())return;if(snap.Recipes.Count==0){Notice("가공법 목록을 불러오지 못했습니다. 게임 연결 상태를 확인하세요.");return;}
        string facility=old==null?Facilities.ForRecipe(cfg,J.S(snap.Recipes[0],"DisplayName")):Facilities.ForRecipe(cfg,old.Name);
        using(var d=new EditDialog("자동 가공 목표")){
            d.Height=680;var facilityChoice=Combo(480);Fill(facilityChoice,Facilities.Names,facility);var recipe=Combo(480);var all=new CheckBox{Text="전체 가공법 보기",AutoSize=true};
            ...
            fill();all.CheckedChanged+=(s,e)=>fill();facilityChoice.SelectedIndexChanged+=(s,e)=>{facility=Convert.ToString(facilityChoice.SelectedItem);fill();};
            ...
            var replenish=new CheckBox{Text="부족 재료 자동 보충 (하위 가공 및 채집 포함)",AutoSize=true,Checked=old==null||old.AutoReplenish};
            d.Add("가공 시설",facilityChoice);d.Content.Controls.Add(all);d.Add("가공법",recipe);d.Add("완제품 (보유 수량 기준)",product);d.Add("가공 방식",mode);d.Add("목표 수량 / 등록 횟수",count);d.Content.Controls.Add(replenish);
            d.Hint("목표 유형과 수량을 설정합니다. 빈 슬롯이 있을 때 설정한 우선순위에 따라 순차적으로 등록됩니다.");
    ...
    void AddManual(){
        ...
        int free=Facilities.ManualAvailable(snap,cfg,facility,reserved);if(free==0){Notice("예약 가능한 빈 슬롯이 없습니다.");return;}
        using(var d=new EditDialog(facility+" · 수동 가공 예약")){d.Height=320;var count=Number(free,free);count.Minimum=1;d.Add("예약 등록 수량 (최대 "+free+"칸)",count);d.Hint("완료품을 먼저 수령한 뒤 남은 빈 슬롯에 1회 일괄 등록합니다.");if(d.ShowDialog(this)!=DialogResult.OK)return;cfg.RecipeFacilities[recipe]=facility;manualQueue.Add(new Goal{Name=recipe,Facility=facility,Target=(int)count.Value});Save();RenderManual();RenderManualInfo();}
    }
    void RenderManualInfo(){if(manualInfoLabel==null)return;string f=CurrentManualFacility();int reserved=manualQueue.Where(x=>(x.Facility??Facilities.ForRecipe(cfg,x.Name))==f).Sum(x=>x.Target);manualInfoLabel.Text="전체 "+Facilities.Total(cfg,f)+" · 진행·대기 "+snap.Works.Count(x=>J.S(x,"FacilityName")==f&&!J.B(x,"IsCompleted"))+" · 예약 가능 "+Facilities.ManualAvailable(snap,cfg,f,reserved);}
    ...
    void RenderGoals(){if(goalGrid==null)return;var selected=Selected<Goal>(goalGrid);goalGrid.Rows.Clear();foreach(var g in cfg.Goals){
        ...
        string state=!g.Enabled?"OFF":met?"목표 달성":Facilities.Free(snap,cfg,Facilities.ForRecipe(cfg,g.Name))==0?"슬롯 대기":RecipeBook.Find(cfg,g.Name)==null?"배합표 없음":Planner.BatchRuns(snap,cfg,g)>0?Planner.BatchRuns(snap,cfg,g)+"회분 준비 중":Reason(recipe);
