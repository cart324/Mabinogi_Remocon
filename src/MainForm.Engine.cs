...
public partial class MainForm {
    ...
    async Task<bool> RunPlan(Plan p,int stamp){
        ...
        try{
            ...
            Reply response;
            if(p.Command=="execute_gathering"&&auto){var wait=await GatherPreemption.Wait(bridge,pending,CollectionFacilities(),()=>auto&&!closing&&!stopping&&!stopRequested&&generation==stamp,StopGatherForCollection,2000,error=>Log("수령 우선 전환 확인 실패 · 현재 채집 응답 대기: "+error));response=wait.Reply;collectionInterrupt=wait.Interrupted;}
            else response=await pending;if(closing)return false;response.Check();Log("게임 명령 응답: "+p.Command+" · "+commandClock.Elapsed.TotalSeconds.ToString("0.00")+"초");var body=J.Unwrap(response.Data);
            string result=J.S(body,"result");if(p.Command=="execute_gathering")SetGatherStatus(GatherResult.Describe(p.Name,body));if(result=="stopped_by_user"){Log(collectionInterrupt?"가공품 수령을 위한 채집 중단 확인":"사용자 중지 · 후속 예약 취소");await RefreshSnapshot(true);return collectionInterrupt&&auto&&!stopRequested&&generation==stamp;}
