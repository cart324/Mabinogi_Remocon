...
public partial class MainForm {
    ...
    Control MusicPanel(string title,Control content){var p=new Panel{Dock=DockStyle.Fill,Margin=new Padding(0,0,6,0)};p.Controls.Add(content);p.Controls.Add(new Label{Text=title,Dock=DockStyle.Top,AutoSize=true,Padding=new Padding(4,4,4,6)});return p;}
    void BuildJukebox(){
        scoreGrid=Grid("악보","보관 위치","잠금");scoreGrid.Columns[0].FillWeight=260;scoreGrid.Columns[1].FillWeight=85;scoreGrid.Columns[2].FillWeight=50;
        instrumentGrid=Grid("악기","보관 위치","장착");instrumentGrid.Columns[0].FillWeight=180;instrumentGrid.Columns[1].FillWeight=110;instrumentGrid.Columns[2].FillWeight=50;
        playlistGrid=Grid("순서","악보","지정 악기","상태");playlistGrid.Columns[0].FillWeight=40;playlistGrid.Columns[1].FillWeight=240;playlistGrid.Columns[2].FillWeight=160;playlistGrid.Columns[3].FillWeight=85;
        ...
        var library=new TabControl{Dock=DockStyle.Fill};var scoresPage=new TabPage("보유 악보 · 가방 → 상자");scoresPage.Controls.Add(scoreGrid);library.TabPages.Add(scoresPage);var instrumentsPage=new TabPage("보유 악기");instrumentsPage.Controls.Add(instrumentGrid);library.TabPages.Add(instrumentsPage);
        ...
        musicStartButton=Button("처음부터 재생",()=>StartMusic(0),true);musicSelectedButton=Button("선택 곡부터 재생",()=>{var selected=Selected<PlaylistEntry>(playlistGrid);if(selected!=null)StartMusic(cfg.Playlist.IndexOf(selected));});musicStopButton=Button("주크박스 중지",RequestMusicStop);
        ...
        musicLibraryStatus=Label("게임 연결 후 보유 목록을 조회합니다.",9);musicLibraryStatus.MaximumSize=new Size(950,0);musicLibraryStatus.Margin=new Padding(4);bar.Controls.Add(musicLibraryStatus);
        Page("주크박스","악보 선택 → 목록 추가 → 곡별 악기 지정 → 재생. 곡이 끝나면 중지 후 다음 곡으로 전환합니다. 같은 이름의 악보·악기는 개별 지정할 수 없습니다.",layout,bar);
        RenderPlaylist();
    }
    async Task RefreshMusic(bool force){
        if(!MusicSupported){musicLibraryStatus.Text="현재 게임에서 주크박스에 필요한 명령을 제공하지 않습니다.";return;}
        ...
        try{
            ...
            musicLibraryStatus.Text="악보 "+scores.Count+"개 · 악기 "+instruments.Count+"개"+(instruments.Any(x=>J.S(x,"Location")=="")?" · 악기 위치는 게임이 제공하지 않습니다.":"");RenderMusicLibrary();RenderPlaylist();
    ...
    void RenderPlaylist(){
        ...
        for(int i=0;i<cfg.Playlist.Count;i++){var entry=cfg.Playlist[i];string state=MusicActive&&jukebox.Index==i?"현재 곡":MusicActive&&jukebox.Index>i?"완료":"대기";if(!MusicActive&&musicCatalogAt!=DateTime.MinValue&&(!musicScores.Any(x=>J.S(x,"DisplayTitle")==entry.Title)||!musicInstruments.Any(x=>J.S(x,"Name")==entry.Instrument)))state="보유 확인 필요";int row=playlistGrid.Rows.Add(i+1,entry.Title,entry.Instrument==""?"악기를 선택하세요":entry.Instrument,state);playlistGrid.Rows[row].Tag=entry;if(entry==selected)playlistGrid.Rows[row].Selected=true;}
        if(first>=0&&first<playlistGrid.Rows.Count)playlistGrid.FirstDisplayedScrollingRowIndex=first;
    }
    bool CanEditMusic(){if(MusicActive||auto||busy||ownsFishing){Notice("현재 주크박스·자동화 작업을 중지한 뒤 목록을 수정하세요.");return false;}return true;}
    void AddSelectedScore(){
        if(!CanEditMusic())return;var score=Selected<object>(scoreGrid);if(score==null){Notice("보유 악보에서 추가할 악보를 선택하세요.");return;}
        ...
        if(entry.Instrument=="")Notice("현재 장착한 악기가 없습니다. ‘곡·악기 수정’에서 악기를 선택하세요.");
    }
    ...
    void EditPlaylistEntry(){
        if(!CanEditMusic())return;var entry=Selected<PlaylistEntry>(playlistGrid);if(entry==null){Notice("플레이리스트에서 수정할 곡을 선택하세요.");return;}
        using(var d=new EditDialog("주크박스 · 곡과 악기")){d.Height=355;var score=Combo(490);Fill(score,musicScores.Select(x=>J.S(x,"DisplayTitle")).Concat(new[]{entry.Title}).Distinct(),entry.Title);var instrument=Combo(490);Fill(instrument,musicInstruments.Select(x=>J.S(x,"Name")).Concat(new[]{entry.Instrument}).Where(x=>x!="").Distinct(),entry.Instrument);if(entry.Instrument=="")instrument.SelectedIndex=-1;d.Add("악보",score);d.Add("이 곡에 사용할 악기",instrument);d.Hint("한 곡이 끝나면 여기 지정한 다음 곡의 악기로 바꿔 연주합니다. 동일 이름의 개별 악기·악보는 구분할 수 없습니다.");if(d.ShowDialog(this)!=DialogResult.OK)return;if(score.SelectedItem==null||instrument.SelectedItem==null){Notice("악보와 악기를 모두 선택하세요.");return;}entry.Title=Convert.ToString(score.SelectedItem);entry.Instrument=Convert.ToString(instrument.SelectedItem);Save();RenderPlaylist();}
    }
    ...
    void StartMusic(int index){
        if(!connected){Notice("게임에 연결한 뒤 재생하세요.");return;}
        if(busy||auto||ownsFishing||actionOwned||MusicActive){Notice("진행 중인 자동화·게임 작업을 중지한 뒤 재생하세요.");return;}
        if(!MusicSupported){Notice("현재 게임에서 주크박스 명령을 지원하지 않습니다.");return;}
        if(snap.Busy(false)){Notice("현재 게임 행동이나 연주를 마친 뒤 재생하세요.");return;}
