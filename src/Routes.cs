...
public partial class MainForm {
    ...
    void BuildRoutes(){
        ...
        bar.Controls.Add(Button("현재 위치로 이탈 확인",()=>{var route=Selected<RoutePreset>(routeGrid);if(route==null)return;if(!connected||snap.At==DateTime.MinValue){Notice("연결된 게임 위치가 필요합니다.");return;}var pos=J.Get(snap.Environment,"WorldPosition");var outside=RouteSharing.IsOutside(route,J.S(snap.Environment,"GameSpaceDisplayName"),J.N(pos,"X"),J.N(pos,"Y"));Notice(!outside.HasValue?"시작점과 허용 경로를 먼저 등록하세요.":(outside.Value?"저장 경로 이탈":"저장 경로 범위 안")+" · "+(int)(DateTime.UtcNow-snap.At).TotalSeconds+"초 전 조회 위치 기준 (이동 명령은 실행하지 않음)");}));bar.Controls.Add(Button("파일 가져오기",ImportRouteFile));bar.Controls.Add(Button("파일 내보내기",ExportRouteFile));
        bar.Controls.Add(Button("공유 문자열 가져오기",ImportRouteCode));bar.Controls.Add(Button("공유 문자열 복사",()=>{try{var r=Selected<RoutePreset>(routeGrid);if(r!=null){Clipboard.SetText(RouteSharing.Code(r));Notice("루트 공유 문자열을 복사했습니다.");}}catch(Exception ex){Notice(ex.Message);}}));
        routeSkillFilter=Combo(150);routeSkillFilter.Items.AddRange(new object[]{"전체","벌목","채광","채집","낚시"});routeSkillFilter.SelectedIndex=0;routeLevelFilter=Number(1,999);routeLevelFilter.Minimum=1;
        routeSkillFilter.SelectedIndexChanged+=(s,e)=>RenderRoutes();routeLevelFilter.ValueChanged+=(s,e)=>RenderRoutes();
        bar.Controls.Add(Label("생활스킬 / 현재 레벨 필터",9));bar.Controls.Add(routeSkillFilter);bar.Controls.Add(routeLevelFilter);bar.SetFlowBreak(routeLevelFilter,true);routeGatherStatus=Label(gatherStatus,10);routeGatherStatus.MaximumSize=new Size(950,0);bar.Controls.Add(routeGatherStatus);
        Page("채집 루트","설계·공유 준비 기능입니다. 현재 API는 좌표 이동·경로 지정·생활스킬 레벨 조회를 지원하지 않아 저장한 루트를 자동 실행하지 않습니다. 실제 채집은 게임의 기본 경로를 사용합니다.",routeGrid,bar);
        RenderRoutes();
    }
    void RenderRoutes(){if(routeGrid==null)return;var selected=Selected<RoutePreset>(routeGrid);string filter=Convert.ToString(routeSkillFilter.SelectedItem);routeGrid.Rows.Clear();
        foreach(var r in cfg.Routes){if(r.Skill!=""&&!routeSkillFilter.Items.Contains(r.Skill))routeSkillFilter.Items.Add(r.Skill);if(filter!="전체"&&(r.Skill!=filter||r.MinLevel>routeLevelFilter.Value||r.MaxLevel<routeLevelFilter.Value))continue;
            int i=routeGrid.Rows.Add(r.Name,r.Material,r.Skill==""?"재료용":r.Skill+" Lv."+r.MinLevel+"–"+r.MaxLevel,r.Points.Count,(r.DeviationMode=="radius"?"시작점 반경 ":"경로 거리 ")+r.Tolerance,RouteSharing.Resolve(cfg,r.Material)==r?"선택됨 (자동 실행 미지원)":"—");routeGrid.Rows[i].Tag=r;if(selected==r)routeGrid.Rows[i].Selected=true;
        }
    }
    void EditRoute(RoutePreset old){if(!CanEdit())return;
        ...
        using(var d=new EditDialog("채집 루트 설정")){d.Height=760;
            ...
            var points=Grid("순서","지역","X","Y");points.Dock=DockStyle.None;points.Size=new Size(490,170);
            ...
            buttons.Controls.Add(Button("현재 위치 추가",()=>{if(snap.At==DateTime.MinValue||!connected){Notice("게임 연결 후 위치를 저장하세요.");return;}var pos=J.Get(snap.Environment,"WorldPosition");r.Points.Add(new RoutePoint{Area=J.S(snap.Environment,"GameSpaceDisplayName"),X=J.N(pos,"X"),Y=J.N(pos,"Y")});render();}));
            ...
            d.Add("루트 이름",name);d.Add("채집 재료 (목록 선택 또는 직접 입력)",material);d.Add("생활스킬 (직접 입력 가능)",skill);d.Add("사용 레벨 구간",levels);d.Add("이탈 판단 방식",deviation);d.Add("이탈 허용 거리 (게임 좌표 단위)",tolerance);d.Add("첫 지점 = 시작·복귀 위치 / 이후 지점 = 허용 경로",points);d.Content.Controls.Add(buttons);d.Hint("시작점 이동 → 채집 → 이탈 시 중지·복귀 → 재시도 구조입니다. 현재 좌표 이동 API가 없어 실행 연동은 준비 단계입니다.");
            ...
            try{RouteSharing.Validate(r);}catch(Exception ex){Notice(ex.Message);return;}
    ...
    bool EditRoutePoint(RoutePreset route,int index){using(var d=new EditDialog("루트 지점")){d.Height=460;var old=index<0?new RoutePoint{Area=""}:route.Points[index];var area=Combo(480);area.DropDownStyle=ComboBoxStyle.DropDown;Fill(area,cfg.Landmarks.Select(entry=>entry.Area).Concat(route.Points.Select(entry=>entry.Area)).Concat(new[]{J.S(snap.Environment,"GameSpaceDisplayName")}).Where(value=>value!="").Distinct(),old.Area);if(old.Area!="")area.Text=old.Area;
    ...
    void ImportRoute(string text){if(!CanEdit())return;var route=RouteSharing.Import(text);cfg.Routes.Add(route);Save();RenderRoutes();Notice("루트를 가져왔습니다. 재료 기본 루트로 선택할 수 있습니다.");}
    void ImportRouteFile(){if(!CanEdit())return;using(var d=new OpenFileDialog{Filter="루트 JSON|*.json"})if(d.ShowDialog(this)==DialogResult.OK)try{if(new FileInfo(d.FileName).Length>100000)throw new IOException("파일이 너무 큽니다.");ImportRoute(File.ReadAllText(d.FileName,Encoding.UTF8));}catch(Exception ex){Notice(ex.Message);}}
    void ExportRouteFile(){var route=Selected<RoutePreset>(routeGrid);if(route==null)return;using(var d=new SaveFileDialog{Filter="루트 JSON|*.json",FileName="route.json"})if(d.ShowDialog(this)==DialogResult.OK)try{File.WriteAllText(d.FileName,RouteSharing.Export(route),new UTF8Encoding(false));}catch(Exception ex){Notice(ex.Message);}}
    void ImportRouteCode(){if(!CanEdit())return;using(var d=new EditDialog("공유 문자열 가져오기")){var text=new TextBox{Multiline=true,ScrollBars=ScrollBars.Vertical,Width=480,Height=220,MaxLength=100000};d.Add("공유 문자열 또는 JSON",text);d.Content.Controls.Add(Button("클립보드 붙여넣기",()=>{try{text.Text=Clipboard.GetText();}catch(Exception ex){Notice(ex.Message);}}));if(d.ShowDialog(this)==DialogResult.OK)try{ImportRoute(text.Text);}catch(Exception ex){Notice(ex.Message);}}}
}
}
