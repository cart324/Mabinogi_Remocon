...
public partial class MainForm {
    ...
    void BuildRoutes(){
        ...
        bar.Controls.Add(Button("공유 문자열 가져오기",ImportRouteCode));bar.Controls.Add(Button("공유 문자열 복사",()=>{try{var r=Selected<RoutePreset>(routeGrid);if(r!=null){Clipboard.SetText(RouteSharing.Code(r));Notice("루트 공유 문자열을 복사했습니다.");}}catch(Exception ex){Notice(UserError(ex));}}));
    ...
    void ImportRouteCode(){if(!CanEdit())return;using(var d=new EditDialog("공유 문자열 가져오기")){var text=new TextBox{Multiline=true,ScrollBars=ScrollBars.Vertical,Width=480,Height=220,MaxLength=100000};d.Add("공유 문자열 또는 JSON",text);d.Content.Controls.Add(Button("클립보드 붙여넣기",()=>{try{text.Text=Clipboard.GetText();}catch(Exception ex){Notice(UserError(ex));}}));if(d.ShowDialog(this)==DialogResult.OK)try{ImportRoute(text.Text);}catch(Exception ex){Notice(UserError(ex));}}}
}
}
